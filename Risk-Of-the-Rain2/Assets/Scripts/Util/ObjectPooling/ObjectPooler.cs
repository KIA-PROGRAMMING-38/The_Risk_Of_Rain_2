...
public class ObjectPooler : MonoBehaviour
{
    ...
    readonly string INFO = " add the information below to this script\nvoid OnDisable()\n{\n" +
        "    ObjectPooler.ReturnToPool(gameObject);    // only once every object \n" +
        "    CancelInvoke();    //  if there's Invoke in Monobehaviour..\n}";
    ...
    void Start()
    {
        ...
        foreach (Pool pool in pools)
        {
            ...
            if (poolDictionary[pool.tag].Count <= 0)
                ...
                Debug.LogError($"{pool.tag}Warning! : Returntool must be one");
        }
    }
